...
import { CalendarIcon, GraduationCap, Shield, Users } from "lucide-react";
...
const Index = () => {
  ...
  const [selectedClass, setSelectedClass] = useState<{ grade: number; classNum: number } | null>(null);
  const [loading, setLoading] = useState(false);
  const [bookedSlots, setBookedSlots] = useState<Array<{ date: string; classLevel: string }>>([]);
  const [isClassDialogOpen, setIsClassDialogOpen] = useState(false);

  const grades = [6, 7, 8, 9];
  const classNumbers = [1, 2, 3, 4, 5, 6];
  ...
  const handleClassSelect = (grade: number, classNum: number) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!teacherName || !subject || !selectedDate || !classLevel || !selectedClass) {
    ...
    try {
      ...
      const classInfo = `${selectedClass.grade}-${selectedClass.classNum}`;
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6" dir="rtl">
              <div className="space-y-2">
                <Label htmlFor="teacherName">اسم المعلمه</Label>
                <Input
                  id="teacherName"
                ...
                <Dialog open={isClassDialogOpen} onOpenChange={setIsClassDialogOpen}>
                  <DialogTrigger asChild>
                    ...
                    >
                      ...
                      {selectedClass ? (
                        <span>الصف {selectedClass.grade} - {selectedClass.classNum}</span>
                      ) : (
                        <span>اختر الصف</span>
                      )}
                    </Button>
                  ...
                  <DialogContent className="max-w-md" dir="rtl">
                    <DialogHeader>
                      <DialogTitle>اختر الصف</DialogTitle>
                      <DialogDescription>
                        اختر الصف الذي ستقوم بتدريسه
                      </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 pt-4">
                      <div className="grid grid-cols-7 gap-2">
                        ...
                        {classNumbers.map((num) => (
                          <div key={num} className="text-center text-xs font-medium text-muted-foreground py-2">
                            {num}
                          </div>
                        ))}
                      </div>
                      {grades.map((grade) => (
                        <div key={grade} className="grid grid-cols-7 gap-2">
                          ...
                          {classNumbers.map((classNum) => (
                            <Button
                              key={`${grade}-${classNum}`}
                              type="button"
                              variant={selectedClass?.grade === grade && selectedClass?.classNum === classNum ? "default" : "outline"}
                              className="h-12"
                              onClick={() => handleClassSelect(grade, classNum)}
                            >
                              {classNum}
                            </Button>
                          ))}
